...
#
#       DMML project  
#       Script   : DMML_ETL_athletes_data.R
#       Author  : Philippe Tap
#       Date: April 2020
#
#-----------------------------------------------------------------------
# This script deals with the ETL of the athletes entries
# It uses :
#    - athlete_events.csv : Olympic athletes file 1896-2016 (271,116 rows)
#    - country_codes.csv : mapping between IOC (Olympic) and ISO standard country codes (229 rows)
#
# Source: https://www.kaggle.com/heesoo37/120-years-of-olympic-history-athletes-and-results
#
# It generates the 'summer_entry_table.csv' file containing the following counts for each country and Olympic
#   - all entries (individual or team) to olympic events 
#   - all distinct medals (gold, silver, bronze, total) 
#-----------------------------------------------------------------------
...
athleteData <- read.csv("data/athlete_events.csv", header=T, na.strings=c(""), stringsAsFactors = F)
...
countryData <- read.csv("data/country_codes.csv", header=T, na.strings=c(""), stringsAsFactors = F)
...
write.csv(medalTable,"data/summer_entry_table.csv", row.names = FALSE, quote=FALSE)
